...
Mode 1: Emergency Mode (isEmergency = true)
Focuses primarily on physical proximity and fast response:
60% Proximity
30% Customer Rating
10% Total Experience


Mode 2: Standard Cooperative Fair-Share (isEmergency = false)
Balances quality, proximity, and equitable distribution of work:
35% Customer Rating
35% Proximity
30% Cooperative Load-Balancing Equity
...
export function rankArtisans({
  ...
  let eligibleWorkers = workers.filter((w) => {
    ...
    if (requiredSkillId || canonicalReqName) {
      const hasSkill = w.skills?.some((ws) => {
        ...
        // Match by Canonical Skill Name
        if (canonicalReqName) {
          const sName = ws.skill?.name || ws.name || (typeof ws === 'string' ? ws : '');
          if (getCanonicalSkillName(sName) === canonicalReqName) {
            return true;
          }
        }
        return false;
  ...
  const scoredWorkers = eligibleWorkers.map((worker) => {
    ...
    // Use PostGIS calculated distance if provided (interpreted in meters -> km or direct km), otherwise fallback to haversineDistance
    let distance;
    if (typeof worker.distanceKm === 'number' && !isNaN(worker.distanceKm)) {
      distance = Math.round(worker.distanceKm * 100) / 100;
    } else if (typeof worker.distance_meters === 'number' && !isNaN(worker.distance_meters)) {
      distance = Math.round((worker.distance_meters / 1000) * 100) / 100;
    } else {
      distance = haversineDistance(userLat, userLng, workerLat, workerLng);
    }
