...
| **Payment Flow (MVP)** | **Built-in Mock Payment System on Bill Page** | Razorpay / Stripe (Future) | Zero external dependency for hackathon MVP demo; allows 1-click test payments on the Bill page that immediately update booking status to `PAID` and demonstrate the 75-10-10-5 split logic. |
